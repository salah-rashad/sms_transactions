...
| IV. Intelligence Through Learning | ✅ PASS | Constitution **v2.0.0** amended Principle IV to mandate fully-dynamic, user-authored parsing with no developer-hardcoded per-sender parsers. The pass-3 design (FR-035) — removing the hardcoded `SmsParser` so user-taught deterministic patterns are the rule-based base layer beneath future learning — is now directly compliant (no reinterpretation needed). The amendment was driven by this feature. |
...
`/speckit-analyze` surfaced four cross-artifact findings; the design was revised to resolve them. (These were later superseded by the pass-3 fully-dynamic pivot — see below. The original I1 "legacy reconciliation" was closed by *removing* the legacy parser entirely, not by consulting it.) Constitution re-check after the pass-3 pivot: **PASS** under Constitution v2.0.0 (Principle IV amended for fully-dynamic parsing).
...
Per an explicit project-owner directive, the hardcoded `SmsParser` and the `AccountSource` enum's parsing role are **removed**; the app parses solely via user-authored patterns (FR-035). This supersedes the earlier R8 legacy-reconciliation approach and closes analyze finding I1 by elimination. Notable existing-code changes: delete `sms_parser.dart`, refactor `Transaction.source` to a sender string, update analytics and `TransactionCubit` to load the ledger from `PatternMatchRepository`, and replace `SmsService.getFinancialSms` with `getCandidateSms`. Constitution Principle IV was amended (v2.0.0) to mandate this fully-dynamic approach, so the design is directly compliant (see Constitution Check, row IV).
